...
def get_next_word(dic, prev_sentence, d_map, t):
    ...
    #cnv_arr = li(waka).map(lambda s: dic.get(s)).list
    #try:
    #    #cnv_arr.remove(dic.get('.'))
    #    #cnv_arr.remove(dic.get(' '))
    #except ValueError:
    #    pass
    #cnv_arr = li(get_wakachi(t, prev_sentence + ".")).map(lambda s: dic.get(s)).list

    #if re.match("^.* $", prev_sentence.replace("\n", "")) != None:
...
def get_cos_sort(trg, d_map):
    ...
    for (k,v) in d_map.items():
        len_d = len(v[0])
        #print(len_d-len_trg)
        #org = zero_ume(v[0], len_trg)
        #cos_val = cos_sim(trg, v[0][len_d-len_trg:])
        if len_trg >= len(v[0]) or v[0][len_trg] == 0:
            continue

        cos_val = cos_sim(trg, v[0][:len_trg])
        #print(f'v: {v[0][len_d-len_trg:]} , val: {cos_val}')
        #print(f'v: {v[0]} , val: {cos_val}')
